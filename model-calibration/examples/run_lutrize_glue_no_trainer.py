...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Whether or not to transpose the weight matrix in linear layer"
    )
    parser.add_argument(
        '--reconstruct_rate',
        type=float,
...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        if args.disable_finetune == False:
            model.train()
            if args.with_tracking:
                total_loss_log = 0
                model_loss_log = 0
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                for name, module in model.named_modules():
                    if isinstance(module, LUTLinear_t):
                        lut_loss += module.lut_loss * args.reconstruct_rate

                # We keep track of the loss at each epoch
                if args.with_tracking:
                    model_loss_log += model_loss.detach().clone()
                    lut_loss_log += lut_loss.detach().clone()
                    total_loss_log = model_loss_log + lut_loss_log

                loss = model_loss + lut_loss
