...
def parse_args():
    ...
    parser.add_argument('--need_measure_energy', action='store_true', default=False)
    parser.add_argument('--need_breakdown', action='store_true', default=False)
    parser.add_argument('--breakdown_type', type=int, default=0)
...
def compile(args):
    ...
    if args.need_breakdown:
        if args.breakdown_type == 0:
            latency_breakdown = 1
            lut_breakdown = 0
            layer_breakdown = 0 
        elif args.breakdown_type == 1:
            latency_breakdown = 0
            lut_breakdown = 1
            layer_breakdown = 0 
        elif args.breakdown_type == 2:
            latency_breakdown = 0
            lut_breakdown = 0
            layer_breakdown = 1
        else:
    ...
    if args.need_measure_energy:
        measure_energy = 1
    else:
        measure_energy = 0

    os.system('mkdir -p build')
    command = f'cmake -DTRANSFORMER=1 -DDYNAMIC=0 -DSEPARATE=0 -DMEASURE_ENERGY={measure_energy} -DLATENCY_BREAKDOWN={latency_breakdown} -DLUT_BREAKDOWN={lut_breakdown} -DLAYER_BREAKDOWN={layer_breakdown} -DNR_TASKLETS={tl_var} \
...
def main():
    ...
    if args.need_compile or args.need_breakdown:
        compile(args)
    if args.need_measure_energy:
        os.system('sudo ./build/bin/test_transformer_layer %s' % args.transformer_config_file)
    else:
        os.system('./build/bin/test_transformer_layer %s' % args.transformer_config_file)
